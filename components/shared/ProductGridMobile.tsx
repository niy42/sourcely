...
export default function ProductGrid() {
    ...
    return (
        <div data-aos="fade-up" className="overflow-hidden mx-auto grid grid-rows-4 gap-8 justify-center max-w-md md:hidden">
            {/* Product Card 1 */}
            <div data-aos="fade-right" className={`relative ${theme === 'dark' ? 'border-none' : ''} h-[450px] w-80 overflow-hidden rounded-lg shadow-xl group hover:shadow-xl hover:-translate-y-2`}>
                        ...
                        <div className="absolute w-full h-full overflow-hidden">
                            {currentLaptop.images.map((img, imgIndex) => (
                                <img
                                    ...
                                    className={`absolute inset-0 mx-auto aspect-square rounded-t-lg object-cover transition-transform transform duration-300 ${imgIndex === currentImageIndexes[0] ? 'translate-x-0 z-10' : '-translate-x-full z-0'}`}
                        ...
                        <div className="absolute w-full h-full overflow-hidden">
                            {currentLaptop.images.map((img, imgIndex) => (
                                <img
                                    ...
                                    src={img.src}
                                    alt={`${currentLaptop.name} image ${imgIndex + 1}`}
                                    className={`absolute inset-0 mx-auto aspect-square rounded-t-lg object-cover transition-transform transform duration-300 ${imgIndex === currentImageIndexes[1] ? 'translate-y-0 opacity-100 visible z-10' : 'translate-y-full opacity-0 invisible z-0'}`}
                                    style={{ transition: 'transform 0.5s ease-in-out, opacity 0.5s ease-in-out' }}
                                />
                            ))}
                        </div>
        ...
        ${hoverStates[1] ? 'over' : 'out'}`}
            ...
            {/* Product Card 3 */}
            <div data-aos="fade-down" className="relative overflow-hidden h-[450px] w-80 rounded-lg shadow-xl group hover:shadow-xl hover:-translate-y-2">
                <Link href="#" className="absolute inset-0 z-10" prefetch={false}>
                    <span className="sr-only">View Product</span>
                </Link>
        ...
        ${hoverStates[2] ? 'over' : 'out'}`}
            ...
            {/* Product Card 4 */}
            <div data-aos="fade-up" className="relative overflow-hidden rounded-lg h-[450px] w-80 shadow-xl group hover:shadow-xl hover:-translate-y-2">
                <Link href="#" className="absolute inset-0 z-10" prefetch={false}>
                    <span className="sr-only">View Product</span>
                </Link>
